...
<br>
```{block, type="info"}
Note that if you're using your own dataframe, you'll have to specify which column contains the strings with the `string_col` argument of the `generate()` function (above, `string_col = "word"`), or else rename the column to be "string".

Custom variables can be neatly integrated with the inbuilt LexOPS dataset (or vice versa) with the [dplyr join functions](https://dplyr.tidyverse.org/reference/join.html). This is how custom variables are used in the Shiny app. See [this section](lexops-shiny-app.html#custom-variables) for a example using custom variables in the shiny app.
```
<br>
