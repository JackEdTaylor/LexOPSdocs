...
This is much slower, as LexOPS will continue trying to generate combinations of items until it has exhausted all the possibilities. We actually generated `r nrow(possible_stim)` words generated per condition with the code above. This number is likely to change slightly each time we run the pipeline, as different combinations are randomly made from all the possible combinations. That said, it is probably a good *indication* of the number of possible stimuli we could generate.
...
<br>
```{block, type="info"}
Custom datasets can also be used in the Shiny app. See [this section](lexops-shiny-app.html#custom-variables) for a example using custom variables in the shiny app.
```
<br>
